...
# 模型文件夹
BOSS_MODEL_DIR = 'srnet_bossbase'
BOWS_MODEL_DIR = 'srnet_bows'
...
def evaluate(model, device, eval_loader, epoch=None, optimizer=None,
             best_acc=None, PARAMS_PATH=None, TMP=None):
  model.eval()
  ...
  # 只有所有训练专用参数均非None时，才执行保存逻辑（即训练模式）
  if all([epoch is not None, optimizer is not None,
          best_acc is not None, PARAMS_PATH is not None,
          TMP is not None]):
    if accuracy > best_acc and epoch > TMP:
      ...
      logging.info(f"Saved new best model at epoch {epoch}")

  logging.info('-' * 8)
  logging.info('Eval accuracy: {:.4f}'.format(accuracy))
  logging.info('-' * 8)
  return accuracy  # 始终返回准确率
...
def test_all_models(args):
  ...
  dataset_config = {
    'BossBase': {
      'cover_dir': str(project_root / 'data' / 'BossBase-256'),
      'train_index': str(project_root / 'index_list' / 'bossbase_train_index.npy'),
      'test_index': str(project_root / 'index_list' / 'bossbase_test_index.npy'),
      'model_dir': 'srnet_bossbase',
      'model_prefix': 'params-BossBase'
    },
    'BOWS2': {
      'cover_dir': str(project_root / 'data' / 'BOWS2-256'),
      'train_index': str(project_root / 'index_list' / 'bossbase_train_index.npy'),
      'test_index': str(project_root / 'index_list' / 'bossbase_test_index.npy'),
      'model_dir': 'srnet_bows',
      'model_prefix': 'params-BOWS2'
    }
  }
  ...
  # 创建结果表格
  results = {
    'BossBase': pd.DataFrame(index=STEGO_ALGORITHMS, columns=STEGO_ALGORITHMS),
    'BOWS2': pd.DataFrame(index=STEGO_ALGORITHMS, columns=STEGO_ALGORITHMS)
  }

  def test_single_model(model, dataset_name, train_algo):
    """测试单个模型在对应数据集上的表现"""
    config = dataset_config[dataset_name]

    # 准备训练集loader用于调整BN统计量
    train_stego_path = os.path.join(data_dir, f'{dataset_name}-{train_algo}')
    ...
    for test_algo in STEGO_ALGORITHMS:
      print(f"Testing {train_algo} model on {dataset_name}-{test_algo}...")
      stego_path = os.path.join(data_dir, f'{dataset_name}-{test_algo}')

      if not os.path.exists(stego_path):
        print(f"警告: {stego_path} 不存在，跳过")
        continue
      ...
      # 执行测试
      model.eval()
      acc = evaluate(model, device, test_loader, None, None, None, None, None)
      print(f"Accuracy: {acc:.4f}")
      results[dataset_name].loc[train_algo, test_algo] = f'{acc:.4f}'

  # 测试所有数据集
  for dataset_name in ['BossBase', 'BOWS2']:
    print(f"\n=== Testing {dataset_name} Models ===")
    config = dataset_config[dataset_name]

    for train_algo in STEGO_ALGORITHMS:
      model_path = os.path.join(
        config['model_dir'],
        f"{config['model_prefix']}-{train_algo}-stego-1-0.4.pt"
      )

      if os.path.exists(model_path):
        print(f"\n--- Testing {train_algo} model ---")
        ...
        # 测试当前数据集
        test_single_model(model, dataset_name, train_algo)

  # 打印和保存结果
  for dataset_name, df in results.items():
    print(f"\n{'=' * 80}")
    print(f"SRNet Model - {dataset_name} Dataset (Embedding Rate: 0.4bpp)".center(80))
    print(f"\n{'=' * 80}")
    ...
    # 保存结果
    df.to_csv(os.path.join(OUTPUT_PATH, f'srnet_{dataset_name.lower()}_results.csv'))
...
def main(args):
  ...
  BOSSBASE_COVER_DIR = './data/BossBase-256'
  BOSSBASE_STEGO_DIR = './data/BossBase-synch'
...
def myParseArgs():
  ...
  parser.add_argument(
    ...
    #required=True
    default='BossBase-synch-stego-1'
  )
