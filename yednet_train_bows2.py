...
class ToTensor():
    def __init__(self, device='cuda', normalize=True):
        self.device = device
        self.normalize = normalize

    def __call__(self, sample):
        # 输入校验（新增）
        if not isinstance(sample, dict) or 'data' not in sample or 'label' not in sample:
            raise ValueError("Sample must be a dict with 'data' and 'label' keys")

        data, label = sample['data'], sample['label']

        # 批量数据处理优化
        if isinstance(data, (list, tuple)):
            data = np.stack([x if isinstance(x, np.ndarray) else np.array(x) for x in data])

        # 统一转换为numpy数组并确保内存连续
        data = np.ascontiguousarray(data, dtype=np.float32)

        # 归一化（可选）
        if self.normalize and np.issubdtype(data.dtype, np.integer):
            data = data / 255.0

        # 自动处理维度 (单样本: [H,W] -> [1,H,W]；批量: [N,H,W] -> [N,1,H,W])
        data = np.expand_dims(data, axis=-3)  # 在倒数第3维（通道维）扩展

        # 处理标签（兼容标量和数组）
        label = np.array(label) if not isinstance(label, (np.ndarray, torch.Tensor)) else label

        # 保持原始代码的返回风格
        new_sample = {
            'data': torch.from_numpy(data),  # 不指定device，保留在CPU
            'label': torch.from_numpy(label).long()
        }

        return new_sample  # 显式返回字典结构
...
def main(args):
  ...
  BOSSBASE_STEGO_DIR = './data/BOWS2-synch'
  ...
  for epoch in range(startEpoch, EPOCHS + 1):
    ...
    if epoch % EVAL_PRINT_FREQUENCY == 0:
      ...
      acc_plot(train_hist, 'yednet_bows', model_name = STEGANOGRAPHY)
