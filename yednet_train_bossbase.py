...
class ToTensor():
    ...
    def __call__(self, sample):
        ...
        return {
            'data': torch.from_numpy(data).float(),  # 显式指定float类型
...
class MyDataset(Dataset):
  ...
  def __getitem__(self, idx):
      ...
      # 显式转换为float32并归一化
      cover = cover.astype(np.float32) / 255.0
      stego = stego.astype(np.float32) / 255.0

      # 打印调试信息
      if idx < 3:
          diff = np.abs(cover - stego).mean()
          print(f"Sample {idx} - Cover vs Stego mean diff: {diff:.2f}")
          print(f"Cover dtype: {cover.dtype}, shape: {cover.shape}, min: {cover.min():.2f}, max: {cover.max():.2f}")

      data = np.stack([cover, stego])
      return {'data': data, 'label': np.array([0, 1], dtype='int32')}
...
def main(args):
  ...
  for epoch in range(startEpoch, EPOCHS + 1):
    ...
    if epoch % EVAL_PRINT_FREQUENCY == 0:
      ...
      acc_plot(train_hist, 'yednet_bossbase', model_name = STEGANOGRAPHY)
